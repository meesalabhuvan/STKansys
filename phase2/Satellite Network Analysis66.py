...
class SatelliteNetworkAnalyzer:
    ...
    def compute_access(self, from_object, to_object, min_elevation_deg=10):
        ...
        if from_object.ClassName == "Facility":
            try:
                # Access the constraints through the proper interface
                access_constraints = access.AccessConstraints
                
                # Try to get elevation angle constraint
                try:
                    elev_constraint = access_constraints.GetActiveConstraint(
                        AgEAccessConstraints.eCstrElevationAngle)
                    elev_constraint.EnableMin = True
                    elev_constraint.Min = min_elevation_deg
                    print(f"  Set minimum elevation: {min_elevation_deg}°")
                except:
                    # If constraint doesn't exist, try adding it
                    print(f"  Using default constraints (elevation constraint not available)")
            except AttributeError:
                # If AccessConstraints doesn't exist, skip constraint setting
